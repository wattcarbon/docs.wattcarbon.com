...
We will be using the [**{{ site.links.m_and_v.seasonal_controls.name }}**]({{ site.links.m_and_v.seasonal_controls.url }}) which uses measured electricity consumption and computes a counterfactual based on maximum daily power consumption. This M&V plan calculates the savings for each device and aggregates them at the site level.
...
An updated CSV template with the correct column names [is available here](https://api.wattcarbon.com/assets/csv/template/energy_efficiency_powder_watts) and a time series description [is available here](https://api.wattcarbon.com/#tag/Meters/operation/Meters-upload_meter_intervals). The daily aggregation data must be uploaded as a document with the columns aligning with the **Daily Aggregated Data** description below. 
...
### Daily Aggregated Data (optional)
...
Assets that have greater than 25 kWh monthly savings differences between the provided aggregated monthly savings and the monthly savings calculated by WattCarbon will be flagged for review.
